...
parole_gst_get_meta_data_cdda (ParoleGst *gst, GstTagList *tag)
{
        ...
        gst_tag_list_get_uint (tag, GST_TAG_TRACK_COUNT, &num_tracks))
    {
        g_object_set (G_OBJECT (gst->priv->stream),
                      ...
                      "track", track,
                      "title", g_strdup_printf(("Track %i"), track),
                      "artist", NULL,
                      "year", NULL,
                      "album", ("Audio CD"),
                      "comment", NULL,
...
parole_gst_check_state_change_timeout (gpointer data)
{
    ...
    if ( gst->priv->state != gst->priv->target )
    {
        ...
        gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), ("Do you want to continue loading or stop?"));
        gtk_dialog_add_button(GTK_DIALOG(dialog), ("Stop"), GTK_RESPONSE_CANCEL);
        gtk_dialog_add_button(GTK_DIALOG(dialog), ("Continue"), GTK_RESPONSE_CLOSE);
...
parole_gst_show_error(GtkWindow *window, GError *error)
{
    ...
    dialog = gtk_message_dialog_new (NULL,
                                     ...
                                     GTK_BUTTONS_CLOSE,
                                     ("GStreamer Error"));
    message = g_strdup_printf("%s\n%s", error->message, ("Parole Media Player cannot start."));
...
parole_gst_constructed (GObject *object)
{
    ...
    if ( G_UNLIKELY (gst->priv->playbin == NULL) )
    {
        ...
        error = g_error_new (1, 0, ("Unable to load \"%s\" plugin"
    ...
    if (g_str_equal (type_name, "AUDIO"))
    {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (tags)
            {
                ...
                {
                    ret = g_list_prepend (ret, g_strdup_printf (("Audio Track #%d"), num++));
                }
        ...
        if (n != 0)
        {
            for (i = 0; i < n; i++)
            {
                ...
                if (tags)
                {
                    ...
                    {
                        ret = g_list_prepend (ret, g_strdup_printf (("Subtitle #%d"), num++));
                    }
                    gst_tag_list_free (tags);
                }
                else

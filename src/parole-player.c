...
void parole_player_set_playlist_visible (ParolePlayer *player, gboolean visibility)
{
    ...
    gtk_widget_set_tooltip_text (GTK_WIDGET(player->priv->showhide_playlist_button), visibility ? ("Hide Playlist"):("Show Playlist"));
...
parole_player_update_chapters (ParolePlayer *player, gint chapter_count)
{
    ...
    {
        menu_item = GTK_WIDGET(gtk_menu_item_new_with_label (g_strdup_printf(("Chapter %i"), chapter_id+1)));
...
parole_player_select_custom_subtitle (GtkMenuItem *widget, gpointer data)
{
    ...
    chooser = gtk_file_chooser_dialog_new (("Select Subtitle File"), NULL,
                                           GTK_FILE_CHOOSER_ACTION_OPEN,
                                           NULL,
                                           NULL);
    gtk_window_set_icon_name(GTK_WINDOW(chooser), "parole");
    button = gtk_dialog_add_button(GTK_DIALOG(chooser), ("Cancel"), GTK_RESPONSE_CANCEL);
    img = gtk_image_new_from_icon_name("gtk-cancel", GTK_ICON_SIZE_BUTTON);
    gtk_button_set_image(GTK_BUTTON(button), img);
    button = gtk_dialog_add_button(GTK_DIALOG(chooser), ("Open"), GTK_RESPONSE_OK);
...
parole_player_recent_menu_clear_activated_cb (GtkWidget *widget, ParolePlayer *player)
{
    ...
    gtk_dialog_add_button (GTK_DIALOG(dlg), ("Cancel"), 0);
    clear_button = gtk_dialog_add_button(GTK_DIALOG(dlg),
                                         "edit-clear",
                                         1);
    gtk_button_set_label( GTK_BUTTON(clear_button), ("Clear Recent Items") );
...
parole_player_seekable_notify (ParoleStream *stream, GParamSpec *spec, ParolePlayer *player)
{
    ...
    gtk_widget_set_tooltip_text (GTK_WIDGET (player->priv->range), seekable ? NULL : ("Media stream is not seekable"));
...
{
    parole_dialog_error (GTK_WINDOW (player->priv->window), ("GStreamer backend error"), error);
    parole_screen_saver_uninhibit (player->priv->screen_saver, GTK_WINDOW (player->priv->window));
    parole_player_stopped (player);
}
...
parole_player_media_tag_cb (ParoleGst *gst, const ParoleStream *stream, ParolePlayer *player)
{
    ...
    if ( player->priv->row )
    {
        ...
        {
            if (year)
                gtk_label_set_markup(GTK_LABEL(player->priv->audiobox_album), g_markup_printf_escaped("<big><span color='#BBBBBB'><i>%s</i></span> <span color='#F4F4F4'>%s (%s)</span></big>", ("on"), album, year));
            else
                gtk_label_set_markup(GTK_LABEL(player->priv->audiobox_album), g_markup_printf_escaped("<big><span color='#BBBBBB'><i>%s</i></span> <span color='#F4F4F4'>%s</span></big>", ("on"), album));

            g_free (album);
        }

        else
            gtk_label_set_markup(GTK_LABEL(player->priv->audiobox_album), g_strdup_printf("<big><span color='#BBBBBB'><i>%s</i></span> <span color='#F4F4F4'>%s</span></big>", ("on"), ("Unknown Album")));
        ...
        if ( artist )
        {
            gtk_label_set_markup(GTK_LABEL(player->priv->audiobox_artist), g_markup_printf_escaped("<big><span color='#BBBBBB'><i>%s</i></span> <span color='#F4F4F4'>%s</span></big>", ("by"), artist));
            g_free (artist);
        }
        else
            gtk_label_set_markup(GTK_LABEL(player->priv->audiobox_artist), g_strdup_printf("<big><span color='#BBBBBB'><i>%s</i></span> <span color='#F4F4F4'>%s</span></big>", ("by"), ("Unknown Artist")));
...
parole_player_buffering_cb (ParoleGst *gst, const ParoleStream *stream, gint percentage, ParolePlayer *player)
{
    ...
    else
    {
        ...
        buff = g_strdup_printf ("%s (%d%%)", ("Buffering"), percentage);
...
parole_player_reset_controls (ParolePlayer *player, gboolean fullscreen)
{
    ...
    if ( player->priv->full_screen != fullscreen )
    {
        ...
        if ( player->priv->full_screen )
        {
            ...
            gtk_menu_item_set_label (GTK_MENU_ITEM(player->priv->fullscreen_menu_item), ("_Fullscreen"));
            gtk_widget_set_tooltip_text (player->priv->fullscreen_button, ("Fullscreen"));
        ...
        else
        {
            ...
            gtk_menu_item_set_label (GTK_MENU_ITEM(player->priv->fullscreen_menu_item), ("Leave _Fullscreen"));
            gtk_widget_set_tooltip_text (player->priv->fullscreen_button, ("Leave Fullscreen"));
...
parole_player_show_menu (ParolePlayer *player, guint button, guint activate_time)
{
    ...
    /*Play menu item
     */
    mi = gtk_menu_item_new_with_mnemonic (player->priv->state == PAROLE_STATE_PLAYING ? ("_Pause"):("_Play"));
     ...
     */
    mi = gtk_menu_item_new_with_mnemonic (("_Previous"));
     ...
     */
    mi = gtk_menu_item_new_with_mnemonic (("_Next"));
    gtk_widget_set_sensitive (mi, (player->priv->state >= PAROLE_STATE_PAUSED));
    gtk_widget_show (mi);
     ...
     * Un/Full screen
     */
    mi = gtk_menu_item_new_with_mnemonic (player->priv->full_screen ? ("_Leave Fullscreen"):("_Fullscreen"));
    gtk_widget_show (mi);
    ...
    if (!player->priv->full_screen)
    {
         ...
         * Un/Hide menubar
         */
        mi = gtk_check_menu_item_new_with_label(("Show menubar"));
...
void parole_player_volume_mute (GtkWidget *widget, ParolePlayer *player)
{
    ...
    if (gtk_scale_button_get_value (GTK_SCALE_BUTTON (player->priv->volume)) == 0.0)
    {
        g_object_get (G_OBJECT (player->priv->conf),
                      "volume", &value,
                      NULL);
        gtk_menu_item_set_label( GTK_MENU_ITEM(player->priv->mute), ("Mute") );
    }
    else
    {
        value = 0;
        gtk_menu_item_set_label( GTK_MENU_ITEM(player->priv->mute), ("Unmute") );
    }
...
on_bug_report_clicked (GtkWidget *w, ParolePlayer *player)
{
    ...
    if (!gtk_show_uri(NULL, "http://docs.xfce.org/apps/parole/bugs", GDK_CURRENT_TIME, NULL))
    {
        ...
        gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog),
                ("Please go to http://docs.xfce.org/apps/parole/bugs to report your bug."));
...
on_goto_position_clicked (GtkWidget *w, ParolePlayer *player)
{
    ...
    /* Create dialog */
    dialog = gtk_dialog_new_with_buttons (("Go to position"),
...
parole_player_init (ParolePlayer *player)
{
    ...
    clear_recent = gtk_menu_item_new_with_mnemonic (("_Clear recent items…"));
    ...
    gtk_widget_set_tooltip_text (GTK_WIDGET (player->priv->previous_button), ("Previous Track"));
    ...
    gtk_widget_set_tooltip_text (GTK_WIDGET (player->priv->next_button), ("Next Track"));
    ...
    gtk_widget_set_tooltip_text (GTK_WIDGET (player->priv->fullscreen_button), ("Fullscreen"));
    ...
    gtk_menu_item_set_label (GTK_MENU_ITEM (player->priv->showhide_playlist_menu_item), ("Show Playlist"));
    ...
    audiotrack_label = gtk_label_new(("Audio Track:"));
    ...
    subtitle_label = gtk_label_new(("Subtitles:"));
    ...
    infobar_close = gtk_button_new_with_label(("Close"));
